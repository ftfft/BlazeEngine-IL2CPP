...
public class NotificationManager : MonoBehaviour
{
    ...
    public static IL2Method methodSendNotification = null;
    ...
    public void SendNotification(string userIdToSendNotificationTo, string type, string message, NotificationDetails notificationDetails)
    {
        ...
        if (method == null)
        {
            unsafe
            {
                ...
                {
                    IntPtr addr = ILCode.GetPointer(instruction);
                    (method = Instance_Class.GetMethod(x => *(IntPtr*)x.ptr == addr && !x.IsStatic && x.GetParameters().Length == 4)).Name = nameof(SendNotification);
                }
            }
        }
