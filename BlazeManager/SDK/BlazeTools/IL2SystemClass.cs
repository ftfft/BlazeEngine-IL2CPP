using System;
using BlazeIL.il2cpp;

namespace BlazeTools
{
    public static class IL2SystemClass
    {
        public static IL2Type Int32 = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Int32", "System");
        public static IL2Type Byte = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Byte", "System");
        public static IL2Type Boolean = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Boolean", "System");
        public static IL2Type Single = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Single", "System");
        public static IL2Type Object = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Object", "System");
        public static IL2Type String = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("String", "System");
        public static IL2Type Type = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Type", "System");
        public static IL2Type action_1 = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("Action`1", "System");
        public static IL2Type vector3 = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.unityenginecoremodule]].GetClass("Vector3", "UnityEngine");
        public static IL2Type quaternion = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.unityenginecoremodule]].GetClass("Quaternion", "UnityEngine");
        public static IL2Type spawnOrientation = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.vrcsdkbase]].GetClass("SpawnOrientation", "VRC.SDKBase.VRC_SceneDescriptor");
        // <-- Attr's --> //
        public static IL2Type compilerGenerated = Assemblies.a[LangTransfer.values[cAssemblies.offset + (long)eAssemblies.mscorlib]].GetClass("CompilerGeneratedAttribute", "System.Runtime.CompilerServices");
    }
}
